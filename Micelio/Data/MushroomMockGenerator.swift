...
private func generateMockMushroom() -> Mushroom {
    ...
    let additionalImages = (0...9).map { "\($0)" }
    
    return Mushroom(name: name, shortDescription: shortDescription, image: image, edibility: edibility, environments: environments, seasons: seasons, description: description, environmentDescription: environmentDescription, edibilityDescription: edibilityDescription, trivia: trivia, additionalImages: additionalImages)
}
