    ...
    var body: some View {
        LazyVGrid(columns: columns, content: {
            ForEach(items, id: \.self) { item in
                LegendItemImage(legendItem: legendItems.filter { $0.icon == item }.first!, dimension: 40, padding: 4, cornerSize: CGSize(width: 5, height: 5), lineWidth: 1)
            }
        })
...
#Preview {
    DetailGrid(items: Season.allCases.map { $0.rawValue })
}
